...
#데이터 학습
model = GaussianHMM(n_components=5,covariance_type="full",n_iter=10,algorithm="viterbi").fit(X)
#state predict
hidden_states = model.predict(X)

#그래프 표현
fig, axs=plt.subplots(3,sharex=True,sharey=False)
plt.suptitle("state prediction(hmm)")
for a,ax in enumerate(axs):
    for i in range(5):
        ...
        for j in range(len(mask)*4//5,len(mask)):
            if mask[j]:
                lx.append(j-len(mask)*4//5)
                ly.append(data[1+j][a])
        ax.scatter(lx,ly,color=colors[i],marker='.',alpha=0.8)
    ax.plot([item[a] for item in data[len(data)*4//5:]],color='black',alpha=0.2)
